...
public:
	...
	virtual void detectJunction_conv(std::vector<Junction> &junctVec);
...
protected:
	...
	void computeStrengthMax(const TMat &GradMagPatch, const TMat &PhasePatch,
		TMat &StrengthMaxVec);

	template<typename TMat>
	void computeStrengthMax_no_phase(const TMat &GradMagPatch, const TMat &PhasePatch,
		std::vector<TMat> &StrengthMaxVec);

	template<typename TMat>
	void computeStrengthMaxLocal(const TMat &GradMagPatch, const TMat &PhasePatch,
		TMat &StrengthMaxVec);
	...
	template<typename TMat>
	void computeStrength_no_phase(const TMat &GradMagPatch, const TMat &PhasePatch,
		const std::vector<Sector> &sectorList, std::vector<float> &strength);
	...
	template<typename TMat>
	bool junctionRefinement_no_phase(float widthAngle,
		const TMat &GradMagPatch,
		const TMat &phasePatch,
		const std::vector<Sector> &sectorVec,
		Junction &junction);
