...
template<typename TMat>
void ACJDetection::computeStrengthMax_no_phase(const TMat &GradMagPatch,
	const TMat &PhasePatch, std::vector<TMat> &StrengthMaxVec)
{
	float SIN;
	size_t cols = GradMagPatch.cols;
	size_t rows = GradMagPatch.rows;

	typedef float FLOAT;
	
	TMat StrengthMax = cv::Mat(rows, cols, CV_32FC1);
	for (int i = 0; i < cols; ++i) {
		for (int j = 0; j < rows; ++j) {
			SIN = fastsin(round_pi(PhasePatch. template at< FLOAT >(j, i) - OrientationPatch.template at< FLOAT >(j, i)));
			StrengthMax. template at< FLOAT >(j, i) = GradMagPatch. template at< FLOAT >(j, i) * pow2(pow2(1 - pow2(SIN)));
		}
	}
}

template<typename TMat>
void ACJDetection::computeStrength_no_phase(const TMat &GradMagPath, const TMat &PhasePatch, 
	const std::vector<Sector> &sectorList, std::vector<float> &strength)
{
		...
		sectorIter != sectorList.end();	++sectorIter)
	{
		...
		for (std::list<cv::Point2i>::const_iterator pointIter = sectorIter->sectPointInd_C.begin();
			pointIter != sectorIter->sectPointInd_C.end();	++pointIter)
		{
			SIN = fastsin(round_pi(PhasePatch.template at< FLOAT >(*pointIter) - sectorIter->Orientation));
			strengthTemp += GradMagPath.template at< FLOAT >(*pointIter) * pow2(pow2(1 - pow2(SIN)));
		}
		strength.push_back(strengthTemp);
	}
}
...
bool ACJDetection::detectJunction(int x, int y, std::list<Junction> &jlist)
{
		...
		sectorListIter != sectorVecVec.end(); ++nsIter, ++sectorListIter)
	{
		...
		{
			computeStrength_no_phase(gradPatch, phasePatch, *sectorListIter, strengthVec);
		}
			...
			tm < branchSize; ++branchIter, ++tm)
		{
			...
			if (!par.useFast)
			{
				ret = junctionRefinement(widthAngle, strengthMax, phasePatch,
					sectorVecVec[nsIter - par.nsMin], junction);
			}
			else
			{
				ret = junctionRefinement_no_phase(widthAngle, gradPatch, phasePatch,
					sectorVecVec[nsIter - par.nsMin], junction);
			}
...
void ACJDetection::detectJunction_conv(std::vector<Junction> &junctVec)
{
	std::cout << "Detecting isotropic scale junctions (fast version)..." << std::endl;
	if (!junctVec.empty())
		junctVec.clear();
	int rows = image.rows;
	int cols = image.cols;
...
template<typename TMat>
bool ACJDetection::junctionRefinement_no_phase(float widthAngle, const TMat &gradMagPatch, 
	const TMat &phasePatch, const std::vector<Sector> &sectorVec, Junction &junction)
{
	...
	for (int i = 0; i < junctionClass; ++i)
	{
		...
		++pointIter)
		{
			...
			typedef float FLOAT;
			float SIN = fastsin(round_pi(phasePatch. template at< FLOAT >(*pointIter) - sectorVec[t].Orientation));
			float weight = gradMagPatch. template at< FLOAT >(*pointIter) * pow2(pow2(1 - pow2(SIN)));
...
int ACJDetection::sketchProposalFixed(int x, int y)
{
	...
	if (!par.useFast)
	{
		...
		computeStrength(strengthMax, sectors, strengthVec);
	}
	else
	{
		computeStrength_no_phase(gradPatch, phasePatch, sectors, strengthVec);
	}
	nms1d_cir(strengthVec, strengthVec.size(), nNeighbor, strengthVecNMS);
